...
void CACHE::prefetcher_cycle_operate()
{
   ...
   int count=0;
   //int check=0;
  if (auto [old_pf_address, stride, degree, distance,distance_prefetch,modified_degree] = lookahead[this]; degree > 0) {
    //auto old_pfd_address = old_pf_address*64;
    //check=1;
    //std::cout << "pf_address  " << degree << std::endl;
   ...
   if(count >= 0)
    ...
    if (virtual_prefetch || (pf_address >> LOG2_PAGE_SIZE) == (old_pf_address >> LOG2_PAGE_SIZE)) {
      ...
      if (success)
        lookahead[this] = {pf_address, stride, degree - 1,distance,distance_prefetch,modified_degree};
...
for (int i = 0; i < sizeof(track_array) / sizeof(track_array[0]); i++) {
  ...
  if (found == set_end) {
    ...
    if(direction >= 0)
    {
      ...
      direction=0;
      //lookahead[this] = {addr, stride, modified_degree,distance,distance_prefetch,modified_degree};
      //std::cout << "address  " << " hi " << std::endl;
    }
    else if(direction < 0)
    {
       distance = -PREFETCH_DISTANCE;
